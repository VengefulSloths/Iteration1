...
public class StatsViewObject extends JComponent {
    ...
    public void paintComponent(Graphics2D g, int x, int y) {
        ...
        g.drawString(statName, x+ horizontalOffset, y + Config.instance().INVENTORY_IMAGE_HEIGHT - 5); //want to draw the item image and then its name on the same line. Drawing of name is offset by the width of the image.
    ...
    public void paintComponent(Graphics2D g, int x, int y, int viewWidth, int viewHeight) {
        ...
        g.drawImage(statIcon, x, y, Config.instance().INVENTORY_IMAGE_WIDTH, Config.instance().INVENTORY_IMAGE_HEIGHT, this);
        g.drawString(statName, x+ (int) newHorizontalOffset, y + Config.instance().INVENTORY_IMAGE_HEIGHT - 5); //want to draw the item image and then its name on the same line. Drawing of name is offset by the width of the image.
    }
